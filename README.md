...
[ブラウザで遊ぶ](https://homura-stack.github.io/haiku-yugishu/) · [ソースコード](https://github.com/homura-stack/haiku-yugishu) · HTML / CSS / Vanilla JavaScript · PC / スマートフォン対応

外部APIや実行時通信に頼らず、二つの入口から三つの遊びへ移動できます。コピペ俳句には90秒で詠む通常モードと、名句を解体して組み替えるハードモード「盗作率鑑定所」があります。和紙・朱・墨を基調にしつつ、それぞれ異なる手触りの俳句体験を一つの作品としてまとめました。
...
「盗作率」は、収録したパブリックドメイン名句12句との**ゲーム内一致率**です。実在の作品や作者に対する盗作判定ではなく、外部検索・生成AI・実行時通信も使いません。

- 語の一致: 50点
- 元句に対する行と順序: 20点
- 読みの近さ: 30点

12句のうち最も近い句との合計を盗作率とし、`100 − 盗作率`をその句のオリジナリティ点として表示します。結果画面では、照合対象になった名句と出典を確認できます。
...
- Nodeテスト: **68件**
- Google Chrome E2E: **6シナリオ**（うち共有札箱の3句完走を390px幅で検証）
- 検証幅: PC（1280×900）／スマートフォン（390×844）
- E2E対象: 3モード導線、再入場、二度の完走、ドラッグ、タップ、キーボード、誤答ロック、保存不能、動きの抑制、`file://`起動、横スクロール、コンソールエラー

`npm.cmd run build:pages` は、GitHub Pagesに配信する `index.html` と `dist/` だけを `pages-artifact/` に生成します。GitHub Actionsでは `main` への反映時にこの成果物を公開します。
